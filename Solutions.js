...
const duplicateCount = (text) => {
  ...
  let state = {letters:[], d:0};
  for(i = 0; i < text.length; i++) {
    let isDuplicated = text.substring(i+1).includes(text[i]);
    let isRecorded = state.letters.includes(text[i]);
    if(isDuplicated && !isRecorded){
      state.d++;
      state.letters.push(text[i])
    }
  }
  return state.d;
}
